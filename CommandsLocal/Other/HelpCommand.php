...
class HelpCommand extends UserCommand
{
     ...
     * @var string
     */
    protected $description = 'Просмотр команд бота';
    ...
    public function execute(): ServerResponse
    {
        ...
        if ($command_str === '') {
            ...
            $text .= PHP_EOL . 'Нравится проект? [Поддержи любой суммой](https://qiwi.com/n/NAZBAV) - деньги пойдут на оплату сервера.' . PHP_EOL;

            if ($safe_to_show && count($admin_commands) > 0) {
                $text .= PHP_EOL . '*Список команд администратора*:' . PHP_EOL;
                foreach ($admin_commands as $admin_command) {
                    $text .= '/' . $admin_command->getName() . ' - ' . $admin_command->getDescription() . PHP_EOL;
                }
            }

            $text .= PHP_EOL . 'Чтобы посмотреть за что отвечает команда: /help <command>';
        ...
        if (isset($all_commands[$command_str]) && ($safe_to_show || !$all_commands[$command_str]->isAdminCommand())) {
            ...
            return $this->replyToChat(sprintf(
                ...
                strtr($command->getUsage(), [' or ' => ' или '])
            ), ['parse_mode' => 'markdown']);
        }

        return $this->replyToChat('Нет информации по команде: `/' . $command_str . '` не существует', ['parse_mode' => 'markdown']);
    }
